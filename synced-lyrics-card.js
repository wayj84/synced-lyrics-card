...
class SyncedLyricsCard extends HTMLElement {
  ...
  constructor() {
    ...
    this._trackKey = ''; this._syncVisible = false; this._syncHideTimer = null; this._syncKey = ''; this._liveOffset = null; this._artKey = ''; this._artUrl = ''; this._artText = ''; this._fetchKey = ''; this._raf = null; this._lastActive = -1; this._translateY = 0;
  }
  setConfig(config) {
  ...
  disconnectedCallback() { cancelAnimationFrame(this._raf); clearTimeout(this._syncHideTimer); }
  ...
  _render() {
    ...
    </style><ha-card><div class="wrap">${c.show_track_info !== false ? `<div class="meta">${track ? this._escape(track.title) + `<span>— ${this._escape(track.artist)}</span>` : 'Synced Lyrics'}</div>` : ''}<div class="viewport">${content}</div>${syncControl}<div class="status">${this._lyrics.length ? 'LRCLIB synced' : this._plainLyrics ? 'LRCLIB lyrics' : ''}</div></div></ha-card>`;
...
class SyncedLyricsCardEditor extends HTMLElement {
  constructor() {
    ...
    this._schema = [
      ...
      {name: 'track_info_font_size', selector: {number: {min:8, max:48, step:1, mode:'box', unit_of_measurement:'px'}}},
      {name: 'header_alignment', selector: {select: {mode:'dropdown', options: [
        {label:'Same as lyrics',value:'inherit'}, {label:'Left',value:'left'}, {label:'Centre',value:'center'}, {label:'Right',value:'right'}]}}},
  ...
  _ensureForm() {
    ...
    this._form.computeLabel = (schema) => ({
      ...
      font_size: 'Lyric font size (px)', font_weight: 'Font weight', card_height: 'Card height (e.g. 560px)', track_info_font_size: 'Title and artist font size (px)', header_alignment: 'Title and artist alignment',
      inactive_opacity: 'Inactive lyric opacity', active_scale: 'Active lyric scale', background_opacity: 'Background opacity', background_mode: 'Background source', artwork_blur: 'Album artwork blur', artwork_opacity: 'Album artwork opacity', artwork_overlay_opacity: 'Album artwork dark overlay', text_color_mode: 'Lyric colour', show_sync_slider: 'Show on-card sync slider', contrast_mode: 'Contrast mode', backdrop_blur: 'Background blur', backdrop_opacity: 'Contrast overlay opacity', text_shadow: 'Text drop shadows', text_shadow_strength: 'Text shadow strength', sync_offset: 'Sync offset (seconds; + advances lyrics, - delays)',
