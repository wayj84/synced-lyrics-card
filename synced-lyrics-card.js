...
class SyncedLyricsCard extends HTMLElement {
  ...
  _render() {
    ...
    </style><ha-card><div class="wrap">${c.show_track_info !== false ? `<div class="meta ${c.header_layout === 'split' || c.header_layout === 'split_reverse' ? `split ${c.header_layout === 'split_reverse' ? 'reverse' : ''}` : ''}">${track ? ((c.header_layout === 'split' || c.header_layout === 'split_reverse') ? `<span class="track-title">${this._escape(track.title)}</span><span class="track-artist">${this._escape(track.artist)}</span>` : `${this._escape(track.title)}<span>— ${this._escape(track.artist)}</span>`) : 'Synced Lyrics'}</div>` : ''}<div class="viewport">${content}</div>${syncControl}<div class="status">${this._lyrics.length ? 'LRCLIB synced' : this._plainLyrics ? 'LRCLIB lyrics' : ''}</div></div></ha-card>`;
