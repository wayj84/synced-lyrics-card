...
class SongInfoCardEditor extends HTMLElement {
 constructor(){super();this._config={};this._hass=null;this._built=false;this._picker=null} setConfig(c){this._config={...SongInfoCard.getStubConfig(),...c};this._build();this._sync()}set hass(h){this._hass=h;this._build();this._sync()}
 _emit(n,v){this._config={...this._config,[n]:v};this.dispatchEvent(new CustomEvent('config-changed',{detail:{config:this._config},bubbles:true,composed:true}))}
 _build(){if(this._built)return;this._built=true;const groups=[['TheAudioDB settings',['audiodb_api_key']],['Show sections',['show_artist_info','show_track_info','show_release_info','show_tags','show_bio','show_links','show_images']],['Appearance',['alignment','font_family','title_font_size','body_font_size','font_weight','background_mode','artwork_blur','artwork_opacity','artwork_overlay_opacity']]],opts={alignment:[['left','Left'],['center','Centre'],['right','Right']],font_family:[['system-ui','System UI'],['Roboto, sans-serif','Roboto'],['Inter, sans-serif','Inter'],['Montserrat, sans-serif','Montserrat'],['Poppins, sans-serif','Poppins'],['serif','Serif']],background_mode:[['theme','Home Assistant theme'],['artwork','Current album artwork']]},checks=new Set(groups[1][1]),nums=new Set(['title_font_size','body_font_size','font_weight','artwork_blur','artwork_opacity','artwork_overlay_opacity']),labels={audiodb_api_key:'TheAudioDB API key',show_artist_info:'Artist information',show_track_info:'Track information',show_release_info:'Release information',show_tags:'Genres and style',show_bio:'TheAudioDB biography',show_links:'TheAudioDB external links',show_images:'TheAudioDB artist image',alignment:'Text alignment',font_family:'Font family',title_font_size:'Title size',body_font_size:'Body size',font_weight:'Title weight',background_mode:'Background source',artwork_blur:'Artwork blur',artwork_opacity:'Artwork opacity',artwork_overlay_opacity:'Artwork overlay'};this.innerHTML='<style>.s{border-top:1px solid var(--divider-color);padding:12px 8px}.s:first-child{border:0}.s h3{margin:0 0 8px;font-size:15px}.r{display:flex;align-items:center;gap:12px;min-height:42px}.r label{flex:1}.r input:not([type=checkbox]),select{width:170px;padding:7px;box-sizing:border-box;background:var(--card-background-color);color:var(--primary-text-color);border:1px solid var(--divider-color);border-radius:4px}ha-entity-picker{display:block;margin:4px 0 12px}</style>';const top=document.createElement('div');top.className='s';top.innerHTML='<h3>Media player</h3>';const picker=document.createElement('ha-entity-picker');picker.label='Media player entity';picker.allowCustomEntity=true;picker.includeDomains=['media_player'];picker.addEventListener('value-changed',e=>this._emit('entity',e.detail?.value||e.target.value));picker.addEventListener('change',e=>this._emit('entity',e.detail?.value||e.target.value));top.append(picker);this.append(top);this._picker=picker;groups.forEach(([g,fs])=>{const sec=document.createElement('div');sec.className='s';const h=document.createElement('h3');h.textContent=g;sec.append(h);fs.forEach(n=>{const r=document.createElement('div');r.className='r';const l=document.createElement('label');l.textContent=labels[n];let e;if(opts[n]){e=document.createElement('select');opts[n].forEach(([v,t])=>e.add(new Option(t,v)))}else{e=document.createElement('input');e.type=checks.has(n)?'checkbox':nums.has(n)?'number':'text';if(nums.has(n)){e.min=0;e.step=n==='font_weight'?100:.01;if(n==='title_font_size'){e.min=14;e.max=72}else if(n==='body_font_size'){e.min=10;e.max=36}else if(n==='artwork_blur')e.max=40}}e.dataset.name=n;e.addEventListener('change',()=>this._emit(n,checks.has(n)?e.checked:nums.has(n)?Number(e.value):e.value));r.append(l,e);sec.append(r)});this.append(sec)})}
 _sync(){if(!this._built)return;if(this._picker){this._picker.hass=this._hass;this._picker.value=this._config.entity||''}this.querySelectorAll('[data-name]').forEach(e=>{const n=e.dataset.name;if(e.type==='checkbox')e.checked=!!this._config[n];else if(document.activeElement!==e)e.value=this._config[n]??''})}
}
