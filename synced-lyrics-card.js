...
class SyncedLyricsCard extends HTMLElement {
  ...
  _render() {
    ...
    const showIntro = c.show_intro !== false && track && this._lyrics.length && this._position() < firstTime;
    ...
    const controls = c.show_media_controls !== false ? `<div class="media-controls"><button data-media="media_previous_track" title="Previous">&#9664;&#9664;</button><button data-media="media_play_pause" class="play" title="Play/Pause">${s?.state === 'playing' ? '&#10074;&#10074;' : '&#9654;'}</button><button data-media="media_next_track" title="Next">&#9654;&#9654;</button><span class="volume">Vol</span><input id="volume-slider" type="range" min="0" max="100" step="1" value="${volume}"><output id="volume-value">${volume}%</output></div>` : '';
