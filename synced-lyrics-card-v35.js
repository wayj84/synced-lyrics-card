...
class SongInfoCardEditor extends HTMLElement {constructor(){super();this._config={};this._built=false}setConfig(c){this._config={...SongInfoCard.getStubConfig(),...c};this._render()}set hass(h){} _emit(n,v){this._config={...this._config,[n]:v};this.dispatchEvent(new CustomEvent('config-changed',{detail:{config:this._config},bubbles:true,composed:true}))}_render(){if(this._built)return;this._built=true;const groups=[['TheAudioDB',['audiodb_api_key','entity']],['Show sections',['show_artist_info','show_track_info','show_release_info','show_tags','show_bio','show_links','show_images']],['Appearance',['alignment','font_family','title_font_size','body_font_size','font_weight','background_mode','artwork_blur','artwork_opacity','artwork_overlay_opacity']]],opts={alignment:['left','center','right'],font_family:['system-ui','Roboto, sans-serif','Inter, sans-serif','Montserrat, sans-serif','serif'],background_mode:['theme','artwork']},checks=new Set(groups[1][1]),nums=new Set(['title_font_size','body_font_size','font_weight','artwork_blur','artwork_opacity','artwork_overlay_opacity']);this.innerHTML='<style>.s{border-top:1px solid var(--divider-color);padding:12px 8px}.s:first-child{border:0}.r{display:flex;align-items:center;gap:12px;min-height:42px}.r label{flex:1}.r input:not([type=checkbox]),select{width:160px;padding:7px;box-sizing:border-box;background:var(--card-background-color);color:var(--primary-text-color);border:1px solid var(--divider-color);border-radius:4px}</style>';groups.forEach(([g,fs])=>{const s=document.createElement('div');s.className='s';s.innerHTML=`<b>${g}</b>`;fs.forEach(n=>{const r=document.createElement('div');r.className='r';const l=document.createElement('label');l.textContent=({audiodb_api_key:'TheAudioDB API key',entity:'Media player',show_artist_info:'Artist information',show_track_info:'Track information',show_release_info:'Release information',show_tags:'Genres and style',show_bio:'Biography',show_links:'External links',show_images:'Artist image',alignment:'Alignment',font_family:'Font family',title_font_size:'Title size',body_font_size:'Body size',font_weight:'Title weight',background_mode:'Background',artwork_blur:'Artwork blur',artwork_opacity:'Artwork opacity',artwork_overlay_opacity:'Artwork overlay'})[n];let e;if(opts[n]){e=document.createElement('select');opts[n].forEach(v=>e.add(new Option(v,v)));e.value=this._config[n]}else{e=document.createElement('input');e.type=checks.has(n)?'checkbox':nums.has(n)?'number':'text';checks.has(n)?e.checked=!!this._config[n]:e.value=this._config[n]??''}e.addEventListener('change',()=>this._emit(n,checks.has(n)?e.checked:nums.has(n)?Number(e.value):e.value));r.append(l,e);s.append(r)});this.append(s)})}}
